...
pub fn movement_func(
    ...
    let cam_rotation = vec2(cam_rotation[0], cam_rotation[1]);
    ...
    if check(map, Key::Named(NamedKey::Space)) {
        cam_pos += cam_up * MOVE_SPEED * delta_time;
    }
    ...
    if check(map, Key::Character(SmolStr::new("w"))) {
        cam_pos += cam_direction * MOVE_SPEED * delta_time;
    }
    if check(map, Key::Character(SmolStr::new("s"))) {
        cam_pos -= cam_direction * MOVE_SPEED * delta_time;
    }
    if check(map, Key::Character(SmolStr::new("a"))) {
        cam_pos += cam_direction.cross(cam_up).normalize() * MOVE_SPEED * delta_time;
    }
